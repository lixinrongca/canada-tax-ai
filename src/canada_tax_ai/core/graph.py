...
from ..tools.tools import  end_node, query_profile, query_cra_rules, save_to_db,query_tax_slips
...
SYSTEM_PROMPT = sys_prompt("user_profile", "v2")
...
def _route_initial(
        ...
        return state.get("next_tool") or "document_node"
        # return "chat_node"
...
def process_chat(user_input: str, thread_id: str = "t4_project"):
    ...
    print(f"🤖 AI: {ai_reply}")
    print(f"Current knowledge base: {current_knowledge}")
...
def process_document(file_path: str,thread_id: str = "doc_project"):
    config = {"configurable": {"thread_id": thread_id}}
