...
#         raise ValueError(f"Unknown province: {code}. Valid: {list(PROVINCES.keys())}")
#     return PROVINCES[code]


def get_provincial_credits(
    province_code: str,
    filing_year: int,
    ...
    has_t4: bool = False,
    has_t5: bool = False,
) -> list[ProvincialCredit]:
    """Return eligible credits for a province filtered by user profile."""
    config = get_province(province_code)
    ...
    return [
        credit for credit in config.credits
        if trigger_map.get(credit.trigger, False)
    ]


def build_provincial_prompt(
    province_code: str,
    filing_year: int,
    **profile_kwargs
) -> str:
    """Build a formatted prompt listing all eligible provincial credits."""
    config = get_province(province_code)
    credits = get_provincial_credits(province_code, filing_year, **profile_kwargs)
    non_auto = [c for c in credits if not c.question.endswith("✅")]

    lines = [
        f"## {config.name} Provincial Tax Credits — {filing_year}",
        f"Basic Personal Amount: ${config.basic_personal_amount:,.0f}",
        "",
        "### Credits to Review:",
    ]
    for c in non_auto:
        lines.append(f"- **{c.title}** ({c.value_hint}): {c.description}")
...
if __name__ == "__main__":
    ...
    for c in credits:
        print(f"{c.title:<45} {c.value_hint}")