...
if __name__ == "__main__":
    ...
    class TaxResult:
        ...
        federal_basic_personal_credit: float = 2355.75
        ...
        total_federal_credits: float = 2355.75
        provincial_basic_personal_credit: float = 0.0
        total_provincial_credits: float = 599.1825
        net_federal_tax: float = 0
        net_provincial_tax: float = 0
        total_payable: float = 0
        total_credits_and_payments: float = 0.0
        balance_owing: float = 0.0
        notes: List[str] = None

    st.markdown("<style>body, .stApp { background: #071221; margin: 0; padding: 0; }</style>", unsafe_allow_html=True)

    result = TaxResult(notes=["Refund: $0.00 — expect ~2 weeks via direct deposit"])
